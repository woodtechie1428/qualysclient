...
def test_perform_request_connection_timeout(qc):
    ...
    with pytest.raises(Timeout) as e_info:
        tr = _util._perform_request(qc,BASE_URI+'/api/2.0/fo/report/?action=list', input_params={'k':'v'})
...
def test_api_request_validation_fail(mocker):
    ...
    with pytest.raises(Exception) as e_info:
        actual = _util._api_request(QualysClient(),BASE_URI+'/api/2.0/fo/report/?action=list', input_params={'k':'v'})
